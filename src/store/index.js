  ...
  triggerExecution: async ({ type, referenceId, environmentId, dataSetId, testCaseIds, label }) => {
    ...
    const runStep = async () => {
      ...
      if (stepIdx >= steps.length) {
        const finalExec = get().executions.find(e => e.id === id);
        const finalStatus = (finalExec?.failed || 0) > 0 ? 'FAILED' : 'PASSED';
      ...
      // Mark step running
      const currentExec = get().executions.find(e => e.id === id);
      if (!currentExec) return;
      const updatedSteps = currentExec.steps.map((st, i) =>
        i === stepIdx ? { ...st, status: 'RUNNING' } : st
      );
      await updateFireDoc(COLLECTIONS.EXECUTIONS, id, {
        status: 'RUNNING',
        progress: Math.round((stepIdx / steps.length) * 100),
        steps: updatedSteps,
      });

      setTimeout(async () => {
        const passed = Math.random() > 0.22;
        const stepDuration = Math.floor(800 + Math.random() * 2500);
        const tcName = steps[stepIdx].name;
        const logs = buildStepLogs(tcName, passed);

        const current = get().executions.find(e => e.id === id);
        if (!current) return;

        const newPassed = passed ? (current.passed || 0) + 1 : (current.passed || 0);
        const newFailed = !passed ? (current.failed || 0) + 1 : (current.failed || 0);

        const newSteps = current.steps.map((st, i) =>
          i === stepIdx ? {
            ...st, status: passed ? 'PASSED' : 'FAILED',
            duration: stepDuration, logs,
            errorMsg: !passed ? logs.find(l => l.level === 'FAIL')?.msg || null : null,
          } : st
        );
        ...
        stepIdx++;
        const latestExec = get().executions.find(e => e.id === id);

        if (!passed && latestExec?.stopOnFailure) {
          const skippedSteps = newSteps.map((st, i) =>
            i >= stepIdx && st.status === 'QUEUED' ? { ...st, status: 'SKIPPED' } : st
          );
          await updateFireDoc(COLLECTIONS.EXECUTIONS, id, {
            status: 'FAILED',
            endTime: new Date().toISOString(),
            duration: Date.now() - new Date(execution.startTime).getTime(),
            skipped: skippedSteps.filter(s => s.status === 'SKIPPED').length,
            progress: 100,
            steps: skippedSteps,
          });
        } else {
