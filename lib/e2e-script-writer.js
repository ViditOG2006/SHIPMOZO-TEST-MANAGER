...
9. Credentials: Use os.getenv("SHIPMOZO_EMAIL") and os.getenv("SHIPMOZO_PASSWORD") for panel credentials (never PANEL_EMAIL or PANEL_PASSWORD).
10. Order / list helpers: Prefer using functions from panel_orders if dealing with orders list:
    - from panel_orders import goto_new_orders_domestic, search_new_orders, reset_new_orders_list_filters
    - Use goto_new_orders_domestic(page, origin) instead of manually navigating via search or sidebar link for order views.
`;
...
Output ONLY the Python file content:`;
  ...
  let code = (result.content || result.text || "").trim();
...
async function generateE2eScript({ executionId, stepId, testCaseName, scriptId, jobId }) {
  ...
  try {
    ...
    // Clean up file if it exists to avoid bypassing AWAITING_SCRIPT on next runs
    try {
      const safeId = scriptId.replace(/[^a-z0-9_]/gi, "_").toLowerCase();
      const filePath = path.join(GENERATED_DIR, `${safeId}.py`);
      if (fs.existsSync(filePath)) {
        fs.unlinkSync(filePath);
        log(`🧹 Cleaned up invalid script file from disk: ${filePath}`);
      }
    } catch (cleanErr) {
      log(`⚠️ Cleanup error: ${cleanErr.message}`);
    }
