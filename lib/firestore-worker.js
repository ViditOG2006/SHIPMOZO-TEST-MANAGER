...
async function resolveExecutionContext(docData) {
  const ctx = { envOverrides: {}, dataEntries: {} };
  ...
  if (docData.environmentId) {
    ...
    if (envDoc) {
      ...
      if (envDoc.baseUrl) ctx.envOverrides.SHIPMOZO_BASE_URL = envDoc.baseUrl;
      if (envDoc.apiUrl) ctx.envOverrides.SHIPMOZO_API_URL = envDoc.apiUrl;
    }
  }
...
async function runExecution(executionId, docData) {
  ...
  for (let i = 0; i < resolvedSteps.length; i++) {
    ...
    // Check if we should skip due to previous failure and stopOnFailure configuration (disabled to never skip subsequent steps on failure)
    if (false && failedCount > 0 && docData.stopOnFailure) {
    ...
    const appId = docData.appId || "APP-001";
    const appDoc = await fetchFirestoreDoc("applications", appId);
    ...
    try {
      ...
      if (step.type === "API") {
        if (backendTester === "Rest Assured") {
          ...
          stepLogs.push({ time: new Date().toISOString(), level: "INFO", msg: `Running Postman collection via Newman (App: ${appDoc?.name || "Shipmozo"})...` });
