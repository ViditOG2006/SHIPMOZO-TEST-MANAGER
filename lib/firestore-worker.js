...
async function resolveExecutionContext(docData) {
  ...
  if (docData.environmentId) {
    const envDoc = await fetchFirestoreDoc("environments", docData.environmentId);
    if (envDoc) {
      const creds = envDoc.credentials || {};
      const username = String(creds.username || "").trim();
      const password = String(creds.password || "").trim();
      if (username && !username.startsWith("(") && username !== "••••••••") {
        ctx.envOverrides.SHIPMOZO_EMAIL = username;
      }
      if (password && password !== "••••••••") {
        ctx.envOverrides.SHIPMOZO_PASSWORD = password;
      }
