      ...
      if (step.type === "API") {
        ...
        const batchOutcome = await runE2eBatch(
          ...
          [scenarioObj],
          {
            runTarget: "frontend",
            recordVideo: false,
            skipAiHeal: !process.env.ANTHROPIC_API_KEY,
          }
        );
        ...
        if (batchOutcome && batchOutcome.ok === false) {
          ...
          stepLogs.push({ time: new Date().toISOString(), level: "INFO", msg: `Playwright output: ${scResult.error || "Success"}` });
          if (scResult.ok === false) {
            stepStatus = "FAILED";
