...
/**
 * Build SHIPMOZO_EMAIL/PASSWORD overrides only when a full credential pair is available.
 ...
 * @returns {{ SHIPMOZO_EMAIL?: string, SHIPMOZO_PASSWORD?: string }}
 */
function resolvePanelCredentialOverrides(envDoc, dataEntries = {}) {
  ...
  const candidates = [
    ...
    {
      email: String(dataEntries.username || dataEntries.SHIPMOZO_EMAIL || "").trim(),
      password: String(dataEntries.password || dataEntries.SHIPMOZO_PASSWORD || "").trim(),
    },
  ];
  ...
  for (const { email, password } of candidates) {
    ...
    if (userOk && passOk) {
      return { SHIPMOZO_EMAIL: email, SHIPMOZO_PASSWORD: password };
    }
  }
  return {};
}
